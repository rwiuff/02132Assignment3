...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
        ...
        \node[state, align=left]         (border) [right = 4 of idle]   {border\\Erosion};
        % \node[state]                     (init)   [below = 4 of idle]   {regInit};
        % \node[state]                     (cond)   [below = 2 of init]   {conditional};

        \path[->]
        (idle)   edge [loop above] node {\textbf{start == 0}}           ( )
        (idle)   edge              node {\textbf{start == 1}}           (border);
    \end{tikzpicture}
    }%
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{The registers containing the image rows}\label{fig:regs}
...
\end{figure}
Furthermore registers are needed to store the memory address, the x and the y counters
